...
def freeze_network(model,sub_networks):
    num_sub_networks = len(sub_networks)

    for name, p in model.named_parameters():
        for i in range(num_sub_networks):
            if sub_networks[i] in name:
...
def un_freeze_network(model,sub_networks):
    num_sub_networks = len(sub_networks)
